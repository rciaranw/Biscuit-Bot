...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("unlockapplications")
        .setDescription("Unlock Staff or Community Helper applications")
    ...
    async execute(interaction) {
        ...
        if (type === "staff") {
            settings.staffApplicationsOpen = true;
        }

        if (type === "helper") {
            settings.helperApplicationsOpen = true;
        }

        await settings.save();

        await interaction.reply({
            content: `${type === "staff" ? "Staff" : "Community Helper"} applications have been unlocked.`,
            ephemeral: true
        });
    }
};